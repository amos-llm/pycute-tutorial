...
from pycute import Layout, idx2crd
...
print(f"id_t = {id_t}")
print(f"id_t(5, 20) = {id_t(5, 20)}")
print(
    f"还原坐标: idx2crd(660, (128,128), (128,1)) = {idx2crd(id_t(5, 20), (M, K), (K, 1))}"
)
...
print(
    f"线程 10 的 (行块内行, 列块) = {idx2crd(10, (16, 8), (8, 1))} -> 行块内行 10//8=1, 列块 10%8=2 (k0=16)"
)
...
print(
    f"线程 {t} 的 8 个行块 (行 = {[thread_value_coord(t, mt, 0, 0)[0] for mt in range(8)]}):"
)
print(f"  谓词: {preds}")
...
print(f"  谓词 k < 48: {[k < d for k in ks]}")
cnt = 0
for t in range(128):
    for mt in range(8):
        for v in range(8):
