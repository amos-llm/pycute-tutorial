...
from pycute import Layout, Swizzle, idx2crd
...
print(
    f"128x128 个 (m,k) 映射到 {len(offs)} 个不同 smem 偏移: {'OK (双射)' if len(offs) == 128 * 128 else 'FAIL'}"
)
...
print(
    f"128 线程 x 2 行带 x 8 块 x 8 值 = {128 * 2 * 8 * 8} 个 (row,k), 覆盖 {len(seen)} 个不同位置: {'OK (恰好一次)' if len(seen) == 128 * 128 else 'FAIL'}"
)
...
for m in range(128):
    for k in range(128):
        ...
        if (m1, k1) != (m, k) or (m2, k2) != (m, k):
            ok = False
print(f"load 与 mma 两条路径对每个 (m,k) 都能无损还原: {'OK' if ok else 'FAIL'}")
