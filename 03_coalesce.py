...
from pycute import Layout, coalesce, flatten, size
...
print(f"L1 = {L1}")
print(f"coalesce(L1) = {coalesce(L1)}")
print(
...
print(f"\nL2 = {L2}")
print(f"coalesce(L2) = {coalesce(L2)}")
print("  # 中间的 1:7 被删掉; 2:1 与 3:2 连续 -> 6:1")

L3 = Layout((12, 12), (1, 12))
print(f"\nL3 = {L3} (列主序方阵)")
print(f"coalesce(L3) = {coalesce(L3)}")

step("3.2 不能合并的例子")
L4 = Layout((4, 4), (1, 8))
print(f"\nL4 = {L4}")
print(f"coalesce(L4) = {coalesce(L4)}  (4*1=4 != 8, 中间有空洞, 合并不了)")
...
print(f"atom = {atom}")
print(f"tile_to_shape(atom, (128,128)) = {tiled}")
print(f"coalesce 后: {coalesce(tiled)}")
print(
