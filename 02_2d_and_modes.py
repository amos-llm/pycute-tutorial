...
from pycute import Layout, cosize, crd2idx, flatten, idx2crd
...
print(f"cosize = {cosize(strided)}  (偏移范围 [0, 30])")

step("2.3 嵌套 mode: 一个 mode 本身是 tuple")
# ((2,2),4):((4,1),2): 第一个 mode 内部还有两个子坐标
N = Layout(((2, 2), 4), ((4, 1), 2))
print(f"N = {N}")
print(f"N 的 rank = {len(N)}")
print(f"第 0 个 mode: {N[0]}  第 1 个 mode: {N[1]}")
print(f"flatten(shape) = {flatten(N.shape)}")
print(f"flatten(stride)= {flatten(N.stride)}")
print(f"N((1,0),3) = {N((1, 0), 3)}  # 1*4 + 0*1 + 3*2 = 10")
print(f"N((0,1),2) = {N((0, 1), 2)}  # 0*4 + 1*1 + 2*2 = 5")

step("2.4 用 None 做切片 (等价于 C++ 的 _)")
L = Layout((4, 8), (8, 1))  # 行主序 4x8
print(f"L(2, None)  = {L(2, None)}  # 第 2 行: 一个 1D 子 layout")
print(f"L(None, 3)  = {L(None, 3)}  # 第 3 列: 步长 8")
print(
    f"第 2 行起始偏移 = crd2idx((2,0),(4,8),(8,1)) = {crd2idx((2, 0), (4, 8), (8, 1))}"
)
...
for t in (0, 5, 10, 127):
    print(
        f"  t={t:>3} -> (row={t // 8}, colblock={t % 8})   [pycute: {idx2crd(t, (16, 8), (8, 1))}]"
    )
