...
print(
    "对比 5.1 的 logical_product: 平铺计数在 tile_to_shape 里是逐原 mode 的独立 mode ((8,16),(64,2)),"
)
print(
    "logical_product 则嵌套成一个 mode ((8,64),(16,2)) —— 两者作为函数相同, 只是 '形状视角' 不同。"
)
...
print(
    "      mode1 = tile 索引 (m_block, 0): 2 个行块 (d 横跨全宽, 所以第二维 size 1), 每行块偏移 16384 = 128*128。"
)
...
print(
    f"logical_divide(24:2, 8) = {ld}  # mode0 = 前 8 个元素 (步长随 A); mode1 = 有多少个这样的块 (步长 = 8*2)"
)
print(
    "by-mode 形式在第 9 章直接用: logical_divide(acc_v, (None, None, 2))  # 末 mode 切半, 其余不动"
)
print("对比: tiler 传 Layout 对象 (而非 shape 元组) 时, 是按 '连续 4096 个元素' 切:")
print(f"  {zipped_divide(Layout((128, 128), (1, 128)), Layout((64, 64)))}")
