...
from pycute import Layout, Swizzle, coalesce, composition, cosize
...
print(f"sV (n,d) = {sV}   sV(n,d) = n*128 + d")
...
B = Layout((d, n), (n, 1))
Vt = composition(sV, B)
print(f"B  (d,n) row-major = {B}")
print(f"Vt = sV o B = {Vt}  (d,n)")
...
print(f"coalesce(Vt) = {coalesce(Vt)}")
...
print(f"两种独立构造一致: swz(Vt(i,j)) == sV_swz(j,i), 对所有 (i,j) 成立: {ok}")
print(
    "# 即 '先转置后 swizzle' == '先 swizzle 后转置': swizzle 只作用于最终偏移, 不影响 '换视角' 的代数。"
)
...
def ex3():
    ...
    print(f"cosize(s) = {cosize(s)} cosize(st) = {cosize(st)}")
    # 2) 它为什么不需要额外内存? (一句话)
    print("你的回答: <在这里写一句话>")
