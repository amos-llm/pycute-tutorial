...
from common import SwizzledLayout, banner, check, exercise, step
...
print(
    f"swz(352) = {swz(352)}  # 352 = 0b101100000, bits[6:9]=101 -> ^ 0b0101000(40) -> 328"
)
...
print(f"base atom      = {base}")
print(f"swizzled atom  = {atom_swz}")
print(f"atom_swz(1, 32) = {atom_swz(1, 32)} = swz(1*64+32) = {swz(96)}")
...
for col8 in (0, 32):
    ...
    print(
        f"         32 个 bank 使用次数全为 1: {all(v == 1 for v in cnt.values())} (无冲突)"
    )
...
print(f"swz 把 [0,512) 映射成它的一个排列: {sorted(img) == list(range(512))}")
