...
from pycute import cosize, crd2idx, idx2crd, Layout, size

banner("第 1 章: Layout 就是函数")

step("1.1 最简单的 layout: 一维")
L = Layout(64)  # Shape=64, 不写 stride 时默认 "列主序" (一维就是连续)
print("L =", L)  # 64:1   读作: 形状 64, 步长 1
print("L(7)  =", L(7))  # 坐标 7 -> 偏移 7*1 = 7
print("size  =", size(L))  # 定义域大小 (输入坐标的个数)
print("cosize=", cosize(L))  # 值域跨度 (输出偏移覆盖的范围)

L2 = Layout(64, 2)  # 步长为 2: 取每隔一个元素
print("\nL2 =", L2)
print("L2(7) =", L2(7))  # 7*2 = 14
print("cosize =", cosize(L2))  # L2(63) = 126, 所以 cosize = 127

step("1.2 二维: offset = coord0*stride0 + coord1*stride1")
# (8,4):(4,1): 8 行 4 列, 行主序 (最后一个 mode 步长为 1)
A = Layout((8, 4), (4, 1))
print("A =", A)
print("A(3,2) =", A(3, 2), "  # 3*4 + 2*1 = 14")
...
""")
print("pycute: Layout((8,4))      =", Layout((8, 4)))
print("pycute: Layout((8,4),(4,1))=", Layout((8, 4), (4, 1)), "  # 显式行主序")
...
def ex3():
    ...
    check(
        f"练习3: 构造 size!=cosize (你的: {your_layout}, "
        f"size={size(your_layout)}, cosize={cosize(your_layout)})",
        ok,
    )
